...
func (l *ServiceAccountUpdateLogic) ServiceAccountUpdate(req *types.ClusterNamespaceResourceUpdateRequest) (resp string, err error) {
	...
	if updateErr != nil {
		...
		_, _ = l.svcCtx.ManagerRpc.ProjectAuditLogAdd(l.ctx, &managerservice.AddOnecProjectAuditLogReq{
			...
			ActionDetail: fmt.Sprintf("用户 %s 在命名空间 %s 更新 ServiceAccount %s 失败, 错误原因: %v", username, req.Namespace, req.Name, updateErr),
			Status:       0,
		})
	...
	_, auditErr := l.svcCtx.ManagerRpc.ProjectAuditLogAdd(l.ctx, &managerservice.AddOnecProjectAuditLogReq{
		ClusterUuid:  req.ClusterUuid,
		Title:        "更新 ServiceAccount",
		ActionDetail: fmt.Sprintf("用户 %s 在命名空间 %s 成功更新 ServiceAccount %s", username, req.Namespace, req.Name),
		Status:       1,
	})
	if auditErr != nil {
