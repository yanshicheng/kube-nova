...
func (l *PVCDeleteLogic) PVCDelete(req *types.ClusterNamespaceResourceRequest) (resp string, err error) {
	...
	if deleteErr != nil {
		...
		_, _ = l.svcCtx.ManagerRpc.ProjectAuditLogAdd(l.ctx, &managerservice.AddOnecProjectAuditLogReq{
			...
			Title:        "删除 PVC",
			ActionDetail: fmt.Sprintf("用户 %s 在命名空间 %s 删除 PVC %s 失败, 错误原因: %v", username, req.Namespace, req.Name, deleteErr),
			Status:       0,
		})
	...
	_, auditErr := l.svcCtx.ManagerRpc.ProjectAuditLogAdd(l.ctx, &managerservice.AddOnecProjectAuditLogReq{
		ClusterUuid:  req.ClusterUuid,
		Title:        "删除 PVC",
		ActionDetail: fmt.Sprintf("用户 %s 在命名空间 %s 成功删除 PVC %s", username, req.Namespace, req.Name),
		Status:       1,
	})
	if auditErr != nil {
