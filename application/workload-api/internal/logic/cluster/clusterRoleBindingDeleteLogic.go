...
func (l *ClusterRoleBindingDeleteLogic) ClusterRoleBindingDelete(req *types.ClusterResourceNameRequest) (resp string, err error) {
	...
	if err != nil {
		...
		l.svcCtx.ManagerRpc.ProjectAuditLogAdd(l.ctx, &managerservice.AddOnecProjectAuditLogReq{
			...
			ActionDetail: fmt.Sprintf("删除 ClusterRoleBinding %s 失败，错误: %v", req.Name, err),
			Status:       0,
		})
	...
	l.svcCtx.ManagerRpc.ProjectAuditLogAdd(l.ctx, &managerservice.AddOnecProjectAuditLogReq{
		ClusterUuid:  req.ClusterUuid,
		Title:        "删除 ClusterRoleBinding",
		ActionDetail: fmt.Sprintf("删除 ClusterRoleBinding %s 成功", req.Name),
		Status:       1,
	})
