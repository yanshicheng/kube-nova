...
func choosePlatformID(reqPlatformID, ctxPlatformID uint64) uint64 {
	if reqPlatformID > 0 {
		return reqPlatformID
	}
	return ctxPlatformID
}
...
func projectToType(in *pb.PortalProject) types.PortalProject {
	if in == nil {
...
func projectsToType(items []*pb.PortalProject) []types.PortalProject {
	data := make([]types.PortalProject, 0, len(items))
