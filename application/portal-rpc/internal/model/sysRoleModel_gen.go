...
var (
	...
	cacheIkubeopsSysRoleIdPrefix   = "cache:ikubeops:sysRole:id:"
	cacheIkubeopsSysRoleCodePrefix = "cache:ikubeops:sysRole:code:"
)
...
type (
	...
	SysRole struct {
		...
		Code      string    `db:"code"`       // 角色编码(如: super, admin, user)
...
func (m *defaultSysRoleModel) Delete(ctx context.Context, id uint64) error {
	...
	ikubeopsSysRoleCodeKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleCodePrefix, data.Code)
	ikubeopsSysRoleIdKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, ikubeopsSysRoleCodeKey, ikubeopsSysRoleIdKey)
	return err
}
...
func (m *defaultSysRoleModel) TransOnSql(ctx context.Context, session sqlx.Session, id uint64, sqlStr string, args ...any) (sql.Result, error) {
	...
	if !isZeroValue(id) {
		...
		ikubeopsSysRoleCodeKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleCodePrefix, data.Code)
		ikubeopsSysRoleIdKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleIdPrefix, id) // 处理缓存逻辑，例如删除或更新缓存
		// 执行带缓存处理的 SQL 操作
		return m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (sql.Result, error) {
			return session.ExecCtx(ctx, query, args...)
		}, ikubeopsSysRoleCodeKey, ikubeopsSysRoleIdKey) // 传递缓存相关的键值
...
func (m *defaultSysRoleModel) FindOne(ctx context.Context, id uint64) (*SysRole, error) {
	ikubeopsSysRoleIdKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleIdPrefix, id)
	var resp SysRole
	err := m.QueryRowCtx(ctx, &resp, ikubeopsSysRoleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultSysRoleModel) FindOneByCode(ctx context.Context, code string) (*SysRole, error) {
	ikubeopsSysRoleCodeKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleCodePrefix, code)
	var resp SysRole
	err := m.QueryRowIndexCtx(ctx, &resp, ikubeopsSysRoleCodeKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where `code` = ? AND `is_deleted` = 0  limit 1", sysRoleRows, m.table)
...
func (m *defaultSysRoleModel) Insert(ctx context.Context, data *SysRole) (sql.Result, error) {
	ikubeopsSysRoleCodeKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleCodePrefix, data.Code)
	ikubeopsSysRoleIdKey := fmt.Sprintf("%s%v", cacheIkubeopsSysRoleIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?)", m.table, sysRoleRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Name, data.Code, data.Remark, data.CreatedBy, data.UpdatedBy, data.IsDeleted)
	}, ikubeopsSysRoleCodeKey, ikubeopsSysRoleIdKey)
	return ret, err
}
...
func (m *defaultSysRoleModel) Update(ctx context.Context, newData *SysRole) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.Name, newData.Code, newData.Remark, newData.CreatedBy, newData.UpdatedBy, newData.IsDeleted, newData.Id)
	}, ikubeopsSysRoleCodeKey, ikubeopsSysRoleIdKey)
	return err
}

func (m *defaultSysRoleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheIkubeopsSysRoleIdPrefix, primary)
}
