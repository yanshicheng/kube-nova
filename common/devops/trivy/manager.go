...
func (Manager) TestConnection(ctx context.Context, req devopstypes.Request) devopstypes.Result {
	...
	metadata.EngineVersion = first(config, "engineVersion", "version", "imageVersion")
	metadata.SchemaVersion = first(config, "dbVersion", "vulnerabilityDbVersion", "schemaVersion")
	if metadata.EngineVersion == "" {
		metadata.EngineVersion = "待配置版本"
	}
	return devopstypes.Healthy("Trivy 配置可用", metadata)
}
