...
func (s *statefulSetOperator) UpdateImage(req *types.UpdateImageRequest) error {
	...
	for i := range statefulSet.Spec.Template.Spec.InitContainers {
		if req.ContainerName == "" || statefulSet.Spec.Template.Spec.InitContainers[i].Name == req.ContainerName {
			statefulSet.Spec.Template.Spec.InitContainers[i].Image = req.Image
			updated = true
	...
	if !updated {
		for i := range statefulSet.Spec.Template.Spec.Containers {
			if req.ContainerName == "" || statefulSet.Spec.Template.Spec.Containers[i].Name == req.ContainerName {
				statefulSet.Spec.Template.Spec.Containers[i].Image = req.Image
				updated = true
				if req.ContainerName != "" {
					break
				}
	...
	if !updated {
		for i := range statefulSet.Spec.Template.Spec.EphemeralContainers {
			if req.ContainerName == "" || statefulSet.Spec.Template.Spec.EphemeralContainers[i].Name == req.ContainerName {
				statefulSet.Spec.Template.Spec.EphemeralContainers[i].Image = req.Image
				updated = true
				if req.ContainerName != "" {
					break
				}
...
func (s *statefulSetOperator) UpdateImages(req *types.UpdateImagesRequest) error {
	...
	for _, img := range req.Containers.InitContainers {
		for i := range statefulSet.Spec.Template.Spec.InitContainers {
			if statefulSet.Spec.Template.Spec.InitContainers[i].Name == img.Name {
				statefulSet.Spec.Template.Spec.InitContainers[i].Image = img.Image
				break
			}
		}
	}
	...
	for _, img := range req.Containers.Containers {
		for i := range statefulSet.Spec.Template.Spec.Containers {
			if statefulSet.Spec.Template.Spec.Containers[i].Name == img.Name {
				statefulSet.Spec.Template.Spec.Containers[i].Image = img.Image
				break
			}
		}
	}
	...
	if req.Containers.EphemeralContainers != nil {
		for _, img := range req.Containers.EphemeralContainers {
			for i := range statefulSet.Spec.Template.Spec.EphemeralContainers {
				if statefulSet.Spec.Template.Spec.EphemeralContainers[i].Name == img.Name {
					statefulSet.Spec.Template.Spec.EphemeralContainers[i].Image = img.Image
					break
				}
			}
		}
